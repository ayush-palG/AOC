...
void reset_wire_bundle(void)
{
  for (size_t i = 0; i < wb.wire_qt; ++i) {
    for (size_t j = 0; j < MAX_ID_SIZE; ++j) wb.wires[i].id[j] = 0;
    wb.wires[i].signal = 0;
  }
}
...
int main(void)
{
  ...
  while (is_all_inst_resolved() == false)
	...
	if (parse_inst(il.insts[i].inst))
	  il.insts[i].is_resolved = true;

  uint16_t a_signal = get_wire_signal("a");
  printf("Signal to wire a before: %d\n", a_signal);

  reset_wire_bundle();
  add_wire("b", a_signal);
  for (size_t i = 0; i < il.inst_size; ++i) {
    il.insts[i].is_resolved = false;
  }
  while (is_all_inst_resolved() == false)
    for (size_t i = 0; i < il.inst_size; ++i)
      if (il.insts[i].is_resolved == false)
  ...
  a_signal = get_wire_signal("a");
  printf("Signal to wire a after: %d\n", a_signal);
